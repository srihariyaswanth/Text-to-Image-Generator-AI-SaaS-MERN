  ...
  return (
    ...
    viewport={{once:true}}>
        ...
        <motion.p className='text-center max-w-xl mx-auto mt-5' 
            initial={{opacity:0,y:20}}
            animate={{opacity:1,y:0}}
            transition={{delay:0.6, duration:0.8}}>Unleash your creativity with AI. 
...
``
        ...
        <motion.div className='flex flex-wrap justify-center mt-16 gap-3'
            ...
            {Array(6).fill('').map((item,index)=>(
                ...
                cursor-pointer max-sw:w-10' whileHover={{scale:1.05,duration:0.1}} src={index%2===0 ? assets.sample_img_1 : assets.sample_img_2} alt="" key={index} width={70}/>
            ))}
        </motion.div>
        <motion.p
        initial={{opacity:0}}
