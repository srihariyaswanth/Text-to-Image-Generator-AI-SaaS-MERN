...
const Buycredit = () => {
  ...
  const paymentRazorpay=async(planId)=>{
    console.log("Calling API:", backendUrl + "/api/user/pay-razor");
      ...
      {plans.map((item,index)=>(
        ...
        className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-600 hover:scale-105 transition-all duration-500'>
          ...
          <p className='mt-6'>
            ...
            <button onClick={()=>paymentRazorpay(item.id)} className='w-full bg-gray-800 text-white mt-8 text-sm rounded-md 
